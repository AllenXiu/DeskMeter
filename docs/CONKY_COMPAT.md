...
| DeskMeter 已注册 | **137** | 96 基础 + 41 个 Conky 扩展（if_*/网络/文本/execbar/lua/磁盘IO/电池） |
| 同名变量（两边都有） | **118** | Conky 语法兼容 |
| DeskMeter 别名/扩展 | **19** | 部分为 Conky 实际存在但未列入文档 yaml |
| Conky 独有（我们未实现） | **311** | 绝大多数为 Linux 专属或 Windows 无对应数据 |
...
| 磁盘 / IO | 0 | （diskio/diskiograph 系列已在 v0.3.0 实现；剩 `disk_protect`/`ioscheduler` Linux 概念） |
| 文本 / 命令细分 | 10 | `cat` `catp` `format_time` `templateN` `texeci` `texecpi` … |
| 条件 / 控制 | 11 | `if_mpd_playing` `if_pa_*` `if_mixer_mute` `blink` 等播放器/音频专属 |
| 音频 / 播放器 | 92 | `audacious_bar` `audacious_bitrate` `audacious_channels` `audacious_filename` `audacious_frequency` `audacious_length` … |
| 硬件 / 温度 / 电池细分 | 49 | `acpi*`/`hwmon`/`i8k_`/`ibm_`/`smapi_bat_*`/`voltage_*`（厂商 ACPI） |
| 网络细分 | 16 | `wireless_*` `tcp_ping` `tcp_portmon` `read_tcp` `read_udp`（Native Wifi/Socket 可做） |
| 邮件 | 12 | `draft_mails` `flagged_mails` `mails` `mboxscan` `new_mails` `pop3_used` … |
| Lua / 其他 | 14 | `lua_bar`/`lua_gauge`/`lua_graph`/`lua_parse` 与 `ical`/`journal`/`mysql`/`sysctl` 等 |
| 系统信息 | 24 | `distribution` `desktop*` `monitor*` `entropy*` `loadgraph` `cpugovernor` 等 |
| 邮件 | 12 | `mails`/`mboxscan`/`flagged_mails` 等（需邮件客户端对接） |
| 未归组 | 12 | `apm_battery_life`/`colorN`/`fontN`/`outlinecolor`/`gid_name` 等 |

## Windows 可替代性评估（未覆盖的 311 个）
...
- **核心监控数据全部覆盖**：CPU/内存/交换/磁盘/网络/进程/温度/时间/主机信息 与 Conky 同名语法一致；if_* 条件、网络信息、文本处理、execbar/graph、lua、磁盘 IO、电池 已按 Windows 替代实现（v0.3.0）。
