...
**设置窗口已移除（用户决策：设置界面干掉，全部功能移至系统托盘）**：刷新间隔/点击穿透/显示器由配置文件驱动（conky.config update_interval + deskmeter 块）；托盘菜单 = 配置▶（切换/导入/重命名/删除/记事本编辑）、开机自启（回托盘）、关于 DeskMeter、退出；InputDialog 保留（命名/重命名）；`--settings` 参数已删除；Top 表头点击排序在 click_through=false 时生效。
...
1. **P0 实现 ✅（`dev` 分支完成，2025 交接更新）**：骨架 / MoonSharp 配置引擎 / 透明置底窗口 / 文本渲染 + 定时刷新全部完成；验收通过（直接加载 `conky-main/data/conky.conf` 渲染，可监测有值、不可监测占位）。运行方式：
   ...
   - **已知边界**：`$cpu N` 单核暂用总占用；行内 `${font}` 解析不生效；scroll 的 right/wait 方向与 graph 的 -t/-l/-x/-y/-m 旗标按简化处理；offset/voffset/tab 仍为空格近似；网络/CPU 首采样为 0；**内存（优化 3 件套已落地）：① LHM 懒加载——仅配置用到 platform/hddtemp 且 temperature!=false 才加载 LibreHardwareMonitor 全家（修复了 temperature=false 无效的 bug），无温度配置 private ~87MB、模块 196→172；② 每 60s GC + SetProcessWorkingSetSize 修剪——工作集 169→45MB（任务管理器观感），private commit 不变；③ 采集器实例重扫周期 60→300 tick。完整温度样本 private ~107MB（内容富），其余配置 <100MB**；鼠标点击小部件受置底（HWND_BOTTOM）限制——被其他窗口覆盖时点击会被上层窗口接收（用户环境实测被全屏 Chrome Legacy Window 拦截）
...
8. **P2 渲染边界项 ✅（dev 分支）**：① **行内 ${font 家族:size=字号} 生效**（FontSpec 解析 + RenderContext.CurrentFont + WidgetText 携带字体，渲染层按段字体绘制，FT 缓存 key 含字体；无参数恢复默认）；② **scroll 方向语义**（left 默认 / right 从尾部向右滚入 / wait 到尾停留后回绕，step 步长 + interval 每 N 次刷新前进）；③ **graph 旗标**（-l 对数刻度 log1p、-m 固定最大值、-i 采样间隔、-y 纵轴倍率，-t/-x 解析忽略；console 回退同步支持）；④ **offset/voffset/tab 像素语义**（WidgetOffset 相对偏移可为负、WidgetVOffset 行内垂直偏移、WidgetTab 像素制表位，由渲染层处理，console 忽略）。新增 12 条单测（FontSpec/行内字体/scroll 三方向/graph 旗标/offset 元素），98/98 全绿；WPF 冒烟渲染正常
...
5. **P2 剩余**：~~主题~~（已取消→多配置，见 6）、~~内存优化~~（见 7）、~~边界项~~（**已完成**，见第 8 条）
